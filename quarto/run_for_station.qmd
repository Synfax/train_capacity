...
The full methodology for how these scores are calculated and why they are important for consideration in upzoning is here.
:::
...
The chart below shows the average patronage on the `r station_lines` line(s) at the last stop before the CBD. It is calculated for two times:
...
The map below shows the properties within 1km of `r station` that are suitable for upzoning in orange, properties that do not meet the conditions described above are black.
...
In terms of area, the most suitable properties cover **`r area_ratio*100`**% of the total area of `r round(sum(by_validity$total_area)/10^6,2)` km\^2.
...
Some points are within the same building, e.g. shopping centres. In these cases, the colour assigned is the most common amenity type.
:::
...
Calculation of the final score for local services uses an index measure. It is described in more detail here.
