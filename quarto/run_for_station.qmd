...
Step 3 is described below, using the target station of `r station_targets`.

Average passenger counts at `r station_targets` across the `r station_lines` line(s) are calculated at two times and directions:
...
There are `r tram_stops_near_station %>% nrow()` tram stops in a 1km radius, the average frequency of trams stopping at these stops is `r mean(tram_f_df$sph) `


and `r bus_stops_near_station %>% nrow()` bus stops, the average frequency of trams stopping at these stops is `r mean(bus_f_df$sph) `
...
A box and jitter plot is displayed below, which seeks to describe how service frequencies are distributed throughout the 1000m radius. Each 'dot' in the plot represents a specific bus or tram stop.

For stations only served by one bus or tram line, the plot is often very constrained and narrow.

```{r warning=FALSE,message=FALSE}
#| echo: false 

tram_f_df = rbind.data.frame(tram_f_df, bus_f_df) %>%
  filter(!is.na(sph))
