...
rect_data <- data.frame(
  Direction = c("Towards Flinders", "Towards Flinders", "Away from Flinders", "Away from Flinders"),  # Your actual directions
  xmin = c(7, 16, 7, 16),
  xmax = c(10, 19, 10, 19),
  ymin = 0,
  ymax = Inf,
  fill = c("morning", "evening", "morning", "evening")
)
...
ggplot() +
  ...
  facet_grid(rows = vars(Direction)) +
  geom_vline(xintercept = c(7,10, 16,19), linetype = 'dashed', color = 'red') +
...
# to_plot <- hourly_factors %>%
#   filter(Station_Name == station) %>%
#   filter(Day_Type == 'Normal Weekend')
#   
