...
Out of `r n_suitable + n_unsuitable` lots within 1km radius of `r station`, `r n_suitable` are easiest to upzone. In terms of area, the easiest properties cover `r area_ratio` of the total area of `r sum(by_validity$total_area) %>% round(.,2)` m^2.
...
While not includes in the model directly, bus route(s) `r bus_route_list` and tram route(s) `r tram_route_list` serve this area.
